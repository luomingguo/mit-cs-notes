...
::: definition 定义 · on-path（旁路）注入式审查
GFW 多为 **on-path**（旁观+注入）而非 **in-path**（串接阻断）：它**复制/镜像**路过的流量做检测，命中规则后**注入伪造分组**来干预——比如向双方注入 **TCP RST** 切断连接，或抢在真实 DNS 应答前**注入伪造 DNS 响应**（DNS 污染）。还会用**主动探测 (active probing)**：发现疑似代理服务器后主动连它、验证它是不是翻墙工具，再封禁。
:::
...
::: definition 定义 · "看起来太随机"本身就是指纹
正常协议（HTTP、TLS 握手等）在头部有大量可识别结构/可打印字符；而全加密代理的首包近似随机比特。GFW 用一组**基于熵/字节分布的启发式**判断一条连接"是不是随机得不像任何已知协议"，是则**实时阻断**。
:::

::: theorem 推论 · 论文逆向出的判定规则与启示
作者通过大量主动测量**逆向出 GFW 的具体判定逻辑**：它对连接**首部字节**算可打印 ASCII 占比、随机性等指标，并设若干**豁免规则 (exemptions)**（比如可打印字符够多、或含常见协议特征就放行）以减少误伤正常流量。**核心启示**：把流量伪装成"纯随机"反而成了独特指纹——更稳健的规避是**模仿一个真实存在的常见协议**（如真 TLS），而不是追求"无特征"。这是审查与规避之间"特征 vs 拟态"的根本张力。
:::

## 三、Wallbleed（NDSS 2025）：GFW 自己的内存泄露漏洞

::: definition 定义 · 类 Heartbleed 的越界读
GFW 的 **DNS 注入子系统**在解析触发审查的 DNS 查询时存在**越界内存读 (out-of-bounds read)**：构造特制的畸形 DNS 查询，能让 GFW 在它**注入的伪造 DNS 响应**里**附带泄露自己进程内存的片段**——就像 Heartbleed 之于 OpenSSL，只不过受害者是一台**国家级审查中间盒**。
:::

::: theorem 推论 · 为什么重要
它让外部研究者得以**窥见 GFW 的内部运行状态**（内存内容），既是难得的"把审查者本身当研究对象"的窗口，也再次印证：**审查基础设施本身就是大型软件，同样有内存安全 bug**（呼应 [[../6.1810 Operating System Engineering/lec2|内存安全]] 的越界读一类问题）。把巨型中间盒部署在关键链路上，其自身漏洞的影响面也是国家级的。
:::
...
::: definition 定义 · 端到端 DNS 解析的形式化模型
论文给 DNS 的**端到端解析过程**建立一个**形式化模型**：精确刻画递归解析器如何沿委派链查询、如何处理记录与缓存，从而可以**严格推理/验证** DNS 在各种配置下的行为是否正确、不同实现是否一致。
:::
...
::: theorem 推论 · 这是一场持续的军备竞赛
审查方升级检测（熵指纹、主动探测、SNI 过滤），规避方升级拟态（模仿真协议、随机化握手、可插拔传输）。没有一劳永逸的解，双方围绕"可识别特征"反复博弈——理解 GFW 的**检测机制与实现细节**（如 §2 逆向、§3 漏洞）正是设计有效规避的前提。
:::
