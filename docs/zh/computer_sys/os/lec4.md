...
::: definition 定义（宏内核的特征）
所有子系统（文件系统、内存分配器、调度器、虚拟内存）都是同一大程序的一部分，且全部以完整硬件权限运行（如 xv6 全程管理态）。
:::
...
::: definition 定义（微内核的关键思想）
内核只提供进程间通信 (*IPC*) 与某种线程/任务概念；文件系统等其他功能都作为**用户态进程/任务**实现。
:::
...
![截屏 2024-09-08 22.31.10](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66ddb5542568a.png)

::: definition L4 的四个基本抽象：
任务（地址空间）、线程、IPC、页映射。
:::
...
::: theorem 推论（中断也走 IPC）
驱动可请求内核打开某设备中断；中断发生时 L4 把它**转换成 IPC 消息**通知相关任务。于是"读写设备 + 获知中断"统一到消息模型里。
:::
...
::: insight 工程联想
鸿蒙早期必须兼容安卓应用，否则生态空转——这与"任何新平台都要先支持既有应用"是同一个网络效应难题。人们不愿为不明显的性能优势从现有系统切换过来。
:::

### 4.2 性能问题

::: example 例题 1
（为什么微内核的 IPC 会比宏内核的系统调用慢？）
:::
...
::: definition 定义（论文主张）
第一代微内核慢且不灵活；第二代（L4）能否克服？把 Linux 移植到 L4 上（L4Linux）与原生 Linux 对比来检验。
:::
...
::: theorem 推论（论文的意义）
它反驳了"微内核必然慢"的成见——只要 IPC/页表切换做得足够快，微内核的模块化与可扩展性收益就能在可接受的性能代价内兑现。
:::
