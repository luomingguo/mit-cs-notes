...
1. 盘上的数据结构是什么？，也就是说，文件系统在磁盘上使用哪些结构来组织它的数据和元数据。 VSFS使用的非常简单的结构，用块或者其他对象组成的数组；而更复杂的FS则使用更复杂的基于树的结构
...
接下来，如何知道哪些 inode 或数据块已经被使用？为此我们使用**bitmap（位图）**。位图的作用是记录：inode 是否已分配、data block 是否已分配，需要两个位图： inode位图和用户数据位图， 一个 4KB 的 bitmap 可以记录32K 个对象是否被分配。但我们的系统只有80个inode和56个数据块，虽然浪费但为了方便起见，我们还是用整整一个块来标记。
...
如果 inode 有12个直接寻址指针，一个间接寻址指针，假设指针大小为4B，则最大文件大小：（12 + 4KB / 4B ) × 4KB = 4144KB
...
因此设计策略是**优化小文件**， 小文件不需要额外的indirect block
