...
DynamoDB 表有多个分片，以处理表的吞吐量和存储需求。每个分区存储表的一个**不重叠且连续的键范围**。每个分片有多个副本，分布在不同的AZ中，以确保高可用性和持久性。这些副本形成一个复制组（replication group）。复制组使用 Multi-Paxos 进行Leader选举和一致性协议，新的领导副本在旧领导副本的租约到期之前，不会处理任何写操作或一致性读取
