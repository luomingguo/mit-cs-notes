...
如图1所示，Facebook将 mc 作为**旁路缓存（look-aside cache）**，实际数据存储在数据库中，应用单独与 Memcached 和数据库通信。Memcached并不知道DB的存在。
...
2， 一个集群中，MC服务器越多，分片就越细，每个Web视图就需要发送更多的数据包（到更多的MC服务器上），即不便于批处理。并且，会出现更多的 in-cast 拥塞，一个 web 页面可能要拉取 20~500 个Keys。客户端必须**并行**发请求，否则总延迟太高。 如果这些请求分散到很多台 memcached 机器，返回时可能几百个数据包同时涌向一个客户端，交换机/NIC 的 buffer 很容易被挤爆（叫做 *in-cast congestion*）
