...
- 包含大量的用户、好有列表、状态、帖子、点赞和照片等数据
- 数据的实时性和一致性并非绝对关键——人们对轻微的延迟是可容忍的
- 读操作占多数（这有助于系统悠哈），但数据的局部性很差（不利于优化）
- 极高的负载：每秒数十亿次存储操作，远超单个存储服务器的处理能力。
  - 比如Mysql每秒能处理十万次简单查询，Memcached每秒处理约一百万次get/put请求
- 跨数据中心的设计（至少西海岸和东海岸）
  - 每个数据中心都包含完整的数据副本、西海岸为主数据中心，其他为MySql异步日志复制的副本
...
## Facebook 应如何使用 Memcached

![image-20241113092518466](http://47.115.50.83:49153/i/6734000120c8d.png)

Facebook使用Memcached作为旁路缓存（look-aside cache）

- 实际数据存储在数据库中，应用单独与 Memcached 和数据库通信。
- Memcached不知道DB的存在

读操作``read(k)``

- 调用``get(k)``获取数据（通过``hash(k)``确定具体的缓存服务器或分片）
- 若``get(k)``结果为空，则从数据库中获取数据，并将结果缓存

写操作``write(k, v)``

- 数据写入DB后，删除缓存中的数据``delete(k)``

### Memcached的好处

- 主要用于读操作、二读操作占总请求的大多数
- 高缓存命中率可以极大减轻DB的负载（ 99% 的命中率，意味着数据库读负载减少了 100 倍）
- 缓存的目标并非减少用户可见的延迟，而是保护数据库服务器避免严重过载

### 键的分配

- 客户端的哈希函数决定了键如何分配到 Memcached 服务器
- 可以进行分片（partition）或复制（replication），或者两者的组合。
- 所有 Web 服务器使用相同的 `hash(k)` 函数，因此若 C1 缓存了键 k，则 C2 也能访问该缓存项。
...
-  内存利用率高（每个键值只存储亿次）
-  适合负载均匀分布的情况
-  缺点：不适合个别件频繁访问的情况。每个 Web 服务器需要与多个缓存服务器通信（高网络开销）。

复制而言：

- 有助于高频访问的键
- 可以在每个数据包中打包多个请求/响应（降低开销）
- 缺点：占用更多内存，从而减少可存储的不同项目多数量，写操作成本增加

## 性能与多区域

> 为什么要多区域即完整的副本？

Solution: 1. 以降低不同地理位置用户的往返时间。2. 本地快速读取，提高访问效率；主数据中心故障时，热备份可以快速恢复。

> 为什么不将用户分区到不同区域？

社交网络的数据关联性强，缺少地理分区的效果。这可能适用于如电子邮件等较少社交关联的场景。
...
- 缓存对于高负载系统至关重要，不仅仅为了减少延迟
- 需要灵活的分区和复制控制工具
- 线性一致性代价过高；最终一致性往往不足够
...
Memcached 是一种广泛知名且简单的内存缓存解决方案。本文描述了 Facebook 如何利用 Memcached 作为构建块，构建并扩展一个分布式KV存储系统，以支持全球最大的社交网络。我们的系统处理每秒数十亿的请求，并存储数万亿的项，以便为全球超过十亿的用户提供丰富的体验。
...
![截屏2024-04-26 00.45.14](http://47.115.50.83:49153/i/662a88a2741f0.png)
...
![截屏2024-04-26 00.56.53](http://47.115.50.83:49153/i/662a8b5b71944.png)
...
![截屏2024-04-26 10.22.05](http://47.115.50.83:49153/i/662b0fd381e16.png)
...
![截屏2024-04-26 10.37.49](http://47.115.50.83:49153/i/662b138230fc2.png)
...
我们用租赁(leases)的方法解决两个问题： 过期集(stale sets)和热点缓存(thundering herds)问题。

- 过期集是指当 Web 服务器在 memcache 中设置一个值时，该值不反映应该缓存的最新值，这可能发生在并发更新 memcache 时发生重新排序时。
- 热点缓存问题是指特定key经历了大量读写活动。由于写活动反复**使**最近设置的值**无效（invalidate）**，许多读取将默认转到更昂贵的路径上。
...
![截屏2024-04-26 17.02.05](http://47.115.50.83:49153/i/662b6d933556a.png)
...
1. 应用程序经常同时获取多个键。
2. 整个数据集适合存储在一个或两个memcached服务器中。
3. 请求速率远高于单个服务器的处理能力。
...
因此，我们将Web服务器和Memcached服务器拆分成多个**前端集群（frontend clusters）**。这些集群与数据库的存储集群一起定义了一个区域。这种区域架构具有更小的故障域和更易于管理的网络配置。我们通过**数据复制来换取更多独立的故障域**以及**减少Incast拥塞**。

### 4.1 区域失效

虽然区域中的存储集群持有数据的权威副本，但用户需求可能会将该数据复制到前端集群中。存储集群负责使缓存数据失效，以保持前端集群与权威版本的一致性。作为优化，当一个Web服务器修改数据时，它还会向其自身集群发送失效请求，以提供**写后读（read-after-write）**语义，并减少本地缓存中过时数据的存在时间。

![截屏2024-09-05 02.28.01](http://47.115.50.83:49153/i/66d8a6b805bd4.png)
...
![image-20241113194550992](http://47.115.50.83:49153/i/6734917182651.png)
...
随着 FaceBook 的服务推广到世界各地，将 web servers 推进到离用户最近的地方能够给用户带来更好的体验；将 FB 的数据中心同步到不同区域 (region)，也能帮助提高 FB 服务的容灾能力；在新的区域可能在各方面产生规模经济效应。因此 memcache 服务也需要能够被部署到多个区域。

![image-20240905011822937](http://47.115.50.83:49153/i/66d8967623734.png)

每个区域包括一个存储集群和几个前端集群。我们指定一个区域作为主数据库所在区域，其他区域则包含只读副本；我们依赖 MySQL 的复制机制来保持副本数据库与主数据库的同步。在这个设计中，Web服务器在访问本地 memcached 服务器或本地数据库副本时会体验到低延迟。当跨多个区域扩展时，保持 memcache 数据与持久存储之间的一致性成为主要技术挑战。这些挑战源于一个单一问题：

**副本数据库可能会滞后于主数据库(复制延迟)**

与许多系统不同的是，memcache 和 Facebook 是一起开发的。这使得应用程序和系统工程师能够共同找到一个既易于应用程序工程师理解，又足够高效和简单以便在规模上可靠运行的模型。我们提供了“尽力而为”的最终一致性，但强调性能和可用性。

**来自主区域的的写入**：我们之前决定通过守护进程要求存储集群使数据失效，这在多区域架构中具有重要作用。特别是，它避免了一个竞争条件，即失效请求比主区域复制数据请求**之前**到达。考虑一个主区域的网络服务器，它在修改数据库后希望使现在过时的数据失效。在主区域内部发送失效请求是安全的。然而，让web服务器在副本区域发送数据失效请求可能会过早，因为数据的更改可能尚未传播到副本数据库。随后来自副本区域查询数据请求会与复制流竞争，从而增加将过时数据写入 memcache 的概率。历史上，我们在扩展到多个区域后实现了 **mcsqueal**。

**来自非主区域的写入**：现在考虑一个用户在非主区域更新其数据时，复制延迟会很大。用户的下一次请求可能会导致混淆，如果他的最近更改缺失。缓存重填只应在复制流跟上后进行。如果没有这样做，后续请求可能会导致从副本获取并缓存过时的数据。

我们采用了**远程标记**机制，以最小化读取过时数据的概率。远程标记的存在表明本地副本数据库中的数据可能已经过时，因此查询应该被重定向到主区域。当一个Web服务器希望更新影响某个Key的数据时，该服务器:

1. 在区域中设置一个远程标记$r_k$
2. 执行写入到主数据库，并将$k$和$r_k$添加到SQL语句中进行失效处理
3. 删除本地集群中的k。

在随后对k的请求中，Web服务器将无法找到缓存中的数据，检查$r_k$是否存在，并根据$r_k$的存在与否将查询重定向到主区域或本地区域。在这种情况下，当发生缓存未命中的情况时，我们显式地以增加延迟为代价，降低读取过时数据的概率。

我们通过使用区域池（ regional pool）来实现远程标记。值得注意的是，这种机制可能会在对同一键的并发修改中暴露过时信息，因为一个操作可能会删除应保持存在的远程标记，而另一个操作仍在进行中。值得强调的是，我们使用memcache来作为远程标记，与缓存结果的用法有些微妙的区别。作为缓存，删除或驱逐键始终是一个安全的操作；它可能会导致数据库负载增加，但不会损害一致性。相反，远程标记的存在有助于区分非主数据库是否持有过时数据。在实际操作中，我们发现远程标记的驱逐和并发修改的情况比较少见。
...
![截屏2024-09-05 01.14.34](http://47.115.50.83:49153/i/66d8959c92e25.png)
...
![image-20240905015956959](http://47.115.50.83:49153/i/66d8a02ec6b11.png)
...
我们通过使用区域池来实现远程标记。注意，这种机制在对同一键的并发修改中可能会揭示过时的信息，因为一个操作可能会删除另一个正在进行操作所需的远程标记。作为缓存，删除或逐出键总是安全的；这可能会导致数据库负载增加，但不会损害一致性。
...
![image-20241114143134175](http://47.115.50.83:49153/i/6735994902b84.png)
...
Ports等人【31】提供了一个库来管理事务性数据库查询的缓存结果。我们的需求需要更灵活的缓存策略。我们对租约（"Leases"）[[18]](https://dl.acm.org/doi/10.1145/74851.74870)和陈旧读取（”stale reads“）【23】的使用，借鉴了高性能系统中缓存一致性和读取操作的先前研究。Ghandeharizadeh和Yap【15】的工作也提出了一种基于时间戳而非显式版本号解决陈旧集问题的算法。
