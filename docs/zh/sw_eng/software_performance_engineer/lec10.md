...
![image-20250612123302873](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/684a588325a26.png)

1. 测量程序 A 的性能表现
2. 对程序 A 进行修改，生成预期更快的改进版程序 A′
3. 测量程序 A′的性能表现
4. 若 A′优于 A，则将 A 更新为 A′版本
5. 若当前程序 A 仍未达到性能要求，则跳转至步骤 2 继续优化
...
得到的测量结果如下图，其中红绿线表示理论数值，x 为实际测量结果

![image-20241003165950579](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66fe5d0b61678.png)
...
Solution: 进行多次 A vs A' 的直接对比测试，并用统计学方法评估结果。考虑假设检验（Null Hypothesis Testing）的方法，默认假设 A 比 A'快（零假设，Null Hypothesis）计算 P 值，P 值越低，越能拒绝零假设（A'确实比 A 快）。

### 测量指标 / 统计量

>  假设你在有一些干扰背景噪音的电脑上对一个确定性程序进行了 100 次性能测量（统计量是耗时时间）。以下什么统计量最能代表这个软件的原始性能呢？
>
>  - 算术平均值
>  - 几何平均
>  - 中位数
...
> ![image-20241004070827780](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66ff23eed154b.png)
>
> 结论： 程序 B 的性能是 A 的三倍以上。这个结论对不对？
>

Solution：

❌错！ s 如果我们看 B/A 的比率，那么 A 比 B 几乎好 3 倍。观察： A/B 的算术平均值并不是 B/A 算术平均值的倒数。即使 A/B 和 B/A 是同样的值互为倒数，但它们的算术平均值却不是倒数关系

![image-20241004071050957](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66ff247e7db8f.png)

✅正确的方法： **几何平均值**

💡观察： 平均值的比率 = 比率的平均值。

![image-20241004071316433](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66ff25104789d.png)
...
- 创建一个程序的 profile，比如
  ...
  - 使用`perf record/report`，基于 OS 和硬件支持的硬件计数器
  - 使用采用技术
...
![image-20241003173435571](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66fe652e106ff.png)

问题：“如果你是一个奥运会手枪教练，你会招募哪位射手加入你的团队？”

答案：“B，因为你只需要教 B 瞄准得更低一些并往左打。”
...
- 代码和数据对齐
  ...
  - 如果跨页边界，TLB 同样
- 线程位置
  - 事实表明，OS 很喜欢用 core0，尽可能不要用 core 0
- 运行时调度器
- 超线程技术
- 租户
- DVFS
...
![image-20241003182725656](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66fe719154b18.png)
...
![image-20241003182849175](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66fe71e5075c6.png)
...
- 断开网络连接
- 不要动鼠标
- 对于串行任务，不要在 core 0 上运行，因为中断程序通常在该核心运行
- 关闭超线程
- 关闭 DVFS
- 关闭 Turbo Boost
- 使用``taskset``将 cilk 工作线程绑定到特定核心
...
![image-20241004052953950](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66ff0cd46f06f.png)

LLVM 通常会对函数进行缓存对齐，但它也提供了几种控制对齐的编译器选项：
