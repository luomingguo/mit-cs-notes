...
在 CSS 中，所有元素都是一个盒子。 CSS 关于盒子如何展示，以及如何影响到其他盒子的
...
`<length>`表示距离的度量单位，有些属性允许负长度，有些则不允许。当值为 0 时，可以省略单位。对于`<length>`的单位，有绝对的，和相对单位。 [更多关于 lengths](https://developer.mozilla.org/en-US/docs/Web/CSS/length)。
...
- 对于相对单位，依赖于其他因素而变化：
  ...
  - 视口相关单位适合用于创建能够随 c 大小自适应缩放的布局。例如， `vh`, `vw`, `vmin`, `vmax`
...
![55860955750](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/55860955750.gif)
