...
![截屏2024-08-21 18.50.08](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66c5c66dd108d.png)
...
>第一步
>
>![截屏2024-06-03 09.42.27](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665d1f89ba981.png)
>
>这是刚开始的状态，运行完第一行。接着，将直接跳到第一次进入循环体，此时全局帧(GF)中，i绑定到0，绑定后结果如图
>
>![截屏2024-06-04 01.36.16](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665dff15c53ee.png)
>
>现在我们进入了 for 循环的主体，接下来我们看到的是第 3-4 行的函数定义 (def)。执行这一语句后，我们将在堆中生成一个新的函数对象，并将其绑定到一个名称。
>
>![截屏2024-06-04 01.38.17](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665dff9061c57.png)
>
>我们创建了函数对象并存储了相关信息。因为我们仍在全局帧中运行，所以这个新函数的封闭帧(enclosing frame)是全局帧，并且名称 func 也绑定在全局帧。
>
>![截屏2024-06-04 01.50.55](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e0284b03f2.png)
>
>到此为止，我们已经到达了 for 循环体的底部，因此我们准备继续下一次迭代。在下次循环时，我们的循环变量 i 将绑定到我们正在循环的 range 对象中的下一个元素
>
>![截屏2024-06-04 01.53.03](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e0305e4380.png)
>
...
>
>![截屏2024-08-21 19.30.27](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/66c5cfdf9e4f9.png)
>
...
>
>![截屏2024-06-04 01.57.12](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e041221a6d.png)
>
...
>
>![截屏2024-06-04 02.06.33](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e062fb6767.png)
>
>现在我们知道了我们要调用哪个函数，以及需要传给这个函数什么值，下一步就是建立新的帧(以及设置他的父指针)。**更新图示，包含一个新帧，并将其父指针指向我们要调用的函数的封闭帧**
>
>![截屏2024-06-04 02.01.25](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e04fb0f171.png)
>
>继续往下，我们有自己的新帧（打上F1标签），下一步就是将函数的参数（这个例子就是x)绑定到传入的参数值。
>
>![截屏2024-06-04 02.03.29](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e057619215.png)
>
...
![截屏2024-06-04 04.23.28](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e2645a15a2.png)

2. ``print('global': x) ``: 我们需要解析并打印x。因为x=1还没执行，第一次输出结果是global: 0

3. ``outer()``：通过解析outer，找到函数对象，这里例子没有参数需要，所以我们直接进入下一步。

![截屏2024-06-04 04.30.23](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e27e46dd80.png)
...
4. 调用outer函数并执行主体， 为此新建一个帧 F1。
   ...
   2. ``def inner()...``语句，创建函数对象inner，其封闭帧指向F1，并将名称inner指向新函数对象
      ...
      ![截屏2024-06-04 04.36.08](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e293ddf64d.png)
   ...
   3. 调用``inner()``，还是一样，解析函数名，和参数。现在我们要新建F2， 将其父指针指向F1。并执行函数主体，我们看到`` print("inner:", x)``，所以我们在 F2 中查找 x,我们在 F2 中找不到 x，所以我们追溯到其父帧 F1，并发现 x 绑定到 1
      ...
      ![截屏2024-06-04 04.43.22](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e2aeeb76dc.png)
   ...
   ![截屏2024-06-04 04.48.31](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e2c26e2d12.png)
...
![截屏2024-06-04 04.51.25](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e2cd54140f.png)
...
>
>![截屏2024-06-04 06.53.03](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e4954b5464.png)
>
>现在我们在 `add_n` 的函数体中最后要做的事情是返回 `inner`。我们将通过指向我们刚刚创建的函数对象来表示返回值。
>
>![截屏2024-06-04 06.54.51](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e49c090417.png)
>
>将add1 绑定到全局帧，并且与刚刚的返回值做关联
>
>![截屏2024-06-04 06.55.16](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e49dd54c52.png)
>
...
>
>![截屏2024-06-04 07.04.56](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e4c1e3e1d6.png)
>
>接着我们调用``add_n(2)``，因此新建F2帧，其n绑定到2，它的父指针指向GF。
>
>![截屏2024-06-04 08.58.14](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e66acc6c17.png)
>
...
>
>![截屏2024-06-04 09.00.02](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e67181a041.png)
>
...
>
>![截屏2024-06-04 09.13.52](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/665e6a57cbae5.png)
