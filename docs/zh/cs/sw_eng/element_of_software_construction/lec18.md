...
在共享内存模型中，并发模块通过**读取和写入共享的可变对象**来进行交互。在单个进程内创建多个线程是共享内存并发的典型例子。由于 TypeScript 没有创建线程的 API，我们再代码实例中使用文件系统作为共享的可变状态
...
HTTP 就是整个 Web 语言，`curl` 是命令行下的 HTTP 客户端，大多数系统自带。
 例如访问 CERN 的第一个网站：
...
## 用 TypeScript 编写 Web 服务器

接下来，我们看看如何用 TypeScript 编写一个简单的 Web 服务器。我们将构建一个示例服务器，它 echo 客户端发来的消息。
...
如前所述，向 Web API 传递参数有多种方式。在我们目前的示例中，`greeting` 作为查询参数传递：
...
一个 Web API 可以包含任意数量的查询参数或路由参数。**路由参数**必须在路径中按特定顺序排列（类似于 Python 和 TypeScript 中的函数参数），而**查询参数**可以按任意顺序提供，但必须包含参数名（类似于 Python 中的 Key 参数）。
