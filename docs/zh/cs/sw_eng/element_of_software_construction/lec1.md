...
type: lecture
lecture: 1
tags: [static-typing, type-system, typescript, immutability]
status: complete
source: 'https://web.mit.edu/6.102/www/sp24/classes/01-static-checking/'
---
# Lec 1 静态检查

## TL;DR

- 静态检查在程序运行前依据类型规则发现错误，能缩短缺陷传播路径，并让类型声明成为可由工具验证的接口文档。
- TypeScript 在 JavaScript 生态上增加静态类型，但外部输入、数值边界和动态行为仍需运行时检查，类型系统不能证明所有业务性质。
- 可变对象与变量重新赋值是不同概念；优先使用不可变值和 `const` 能减少别名与状态变化带来的推理负担。
- 静态类型同时服务于避免错误、易于理解和便于修改，但语言选择仍应服从问题、生态和团队约束。
...
::: insight
类型系统最有价值的地方不是减少键盘输入，而是把一部分设计约束变成持续执行的检查。它尤其擅长保护模块边界和重构过程；对于外部数据是否合法、数值是否符合业务范围等语义问题，仍需要规格、验证和测试共同承担。
:::
