...
type: lecture
lecture: 3
tags: [code-review, refactoring, code-quality, maintainability]
status: complete
source: 'https://web.mit.edu/6.102/www/sp24/classes/03-code-review/'
---
# Lec 3 代码审查

## TL;DR

- 代码审查由非原作者系统检查代码，既发现缺陷和可维护性风险，也让团队共享设计知识与工程习惯。
- DRY、清晰命名、必要注释、快速失败、避免全局变量和返回结果而非打印，都会降低代码理解与修改成本。
- 代码异味通常指向职责混乱或错误边界，而不只是格式问题；评审应同时检查行为、结构、可读性和测试证据。
- 重构必须用小步修改、持续测试和版本控制保护行为等价，避免把“清理代码”变成一次不可验证的大改。
...
::: insight
代码审查不只是合并前的质量闸门，也是团队建立共同设计语言的反馈回路。高价值评论应说明风险和约束，例如“这个全局状态会让测试相互影响”，而不是只给出个人风格偏好；能由格式化器或静态检查器决定的问题应交给工具。
:::
