...
type: lecture
lecture: 10
tags: [equality, hash-function, object-identity, mutability]
status: complete
source: 'https://web.mit.edu/6.102/www/sp24/classes/10-equality/'
---
# Lec 10 相等性

## TL;DR

- 表示相等比较内部字段，观察相等比较当前可见结果，行为相等则要求对象未来对所有操作都不可区分；不同客户端可能需要不同定义。
- 不可变值适合按抽象值定义相等性，可变对象的值相等会随状态变化，因此身份相等通常更适合作为哈希容器键的基础。
- 哈希函数必须与相等关系保持一致：若两个值相等，它们必须产生相同哈希值，否则集合和映射会出现不可预测行为。
- Python、JavaScript 与 TypeScript 对原始值、集合和用户定义类型采用不同默认语义，ADT 设计者不能依赖语言直觉代替明确规格。
...
::: insight
相等性不是对象自带的唯一事实，而是客户端观察模型的一部分。若一个类型要作为集合元素、缓存键或数据库标识，应先明确相等关系是否会随时间变化，再设计哈希、不可变性与身份策略；否则“相等”会在不同调用场景中产生冲突。
:::
