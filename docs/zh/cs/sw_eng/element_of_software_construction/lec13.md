...
type: lecture
lecture: 13
tags: [debugging, fault-localization, assertion, reproducibility]
status: complete
source: 'https://web.mit.edu/6.102/www/sp24/classes/13-debugging/'
---
# Lec 13 调试

## TL;DR

- 最低成本的调试策略是让错误无法发生：静态检查、动态检查、不可变性和封装都能提前排除一类缺陷。
- 快速失败、断言、增量开发、单元测试和小作用域让错误更接近根因暴露，从而缩小需要搜索的状态与代码范围。
- 对难复现问题应先稳定复现并记录事实，再用科学方法提出假设、设计无害探针和二分实验逐步定位。
- 修复应处理根因并补回归测试，而不是只消除表面症状；调试日志、版本控制和适时休息都是信息管理工具。
...
::: insight
调试的核心是最大化每次实验获得的信息，而不是不断修改代码碰运气。先固定可复现输入，再只改变一个变量并记录结果；当观察会影响时序时，应改用可控日志、事件记录或确定性测试环境，避免探针本身改变故障。
:::
