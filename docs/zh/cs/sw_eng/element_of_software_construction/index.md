...
course_id: '6.1020'
tags: [software-construction, specification, testing, abstract-data-type, concurrency]
status: draft
source: 'https://web.mit.edu/6.102/www/sp24/'
---
# 6.1020 软件构造基础

## TL;DR

- 课程围绕“避免错误、易于理解、便于修改”三项目标，建立从静态检查、测试和规格到数据抽象与并发设计的软件构造方法。
- 前半部分强调用类型、测试、代码审查和行为规格控制局部复杂度；中段用抽象数据类型、表示不变式与函数式编程组织可演进模块。
- 后半部分讨论递归数据类型、解析、调试以及 Promise、互斥和消息传递，说明如何在异步和并发环境中维护规格与不变式。
- 推荐按讲次顺序学习并配合练习；当前课程索引与 Lec 11、Lec 12 仍有明确内容缺口，因此课程状态保持为 draft。
