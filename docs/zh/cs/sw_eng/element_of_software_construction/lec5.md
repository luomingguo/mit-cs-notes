...
type: lecture
lecture: 5
tags: [specification-design, nondeterminism, mutability, aliasing, contract-design]
status: complete
source: 'https://web.mit.edu/6.102/www/sp24/classes/05-designing-specs/'
---
# Lec 5 设计规范

## TL;DR

- 非确定性规格只约束必须成立的性质，允许多个合法结果，从而为实现者保留算法、性能和数据结构上的选择空间。
- 声明式规格描述“结果应满足什么”，操作式规格描述“如何得到结果”；公共接口通常应优先使用更稳定的声明式表达。
- 更强的后置条件和更弱的前置条件让规格对客户端更有利，但也增加实现成本，设计时需要平衡可用性与实现自由。
- 可变对象和别名会扩大副作用的可观察范围，因此规格必须明确哪些对象可能改变，以及失败时是否留下部分修改。
...
::: insight
好的规格通常比当前实现更抽象、比客户端的隐含假设更明确。客户端越多、实现越多、维护周期越长，声明式行为边界就越重要；私有辅助函数则可以采用更窄、更贴近实现的契约，以降低不必要的文档和兼容成本。
:::
