...
type: lecture
lecture: 12
tags: [grammar, parsing, regular-expression, syntax-tree]
status: draft
source: 'https://web.mit.edu/6.102/www/sp24/classes/12-grammars-parsing/'
---
# Lec 12 语法 & 解析

## TL;DR

- 文法用终结符、非终结符、产生式和组合运算符定义一组合法序列，是文件格式、网络协议和命令行接口等序列规范的形式化表达。
- 解析器依据文法把字符序列转换为语法树；语法树保留产生式结构，通常会进一步转换为程序使用的递归数据类型。
- 正则文法可以消去根以外的非终结符并写成正则表达式，但包含不可消除递归的文法不能由正则表达式完整描述。
- 当前笔记已覆盖文法、语法树和正则表达式；解析器生成器及语法树到抽象数据类型的转换仍待补充。
...
这种由文法生成的数据结构通常是**递归数据类型**，可与上一讲的[递归数据类型](./lec11.md)联系起来理解。
...
例如，一个只允许单个特定 URL 的文法（即只包含一个字符串的集合）可能只有一个产生式：
...
- 可选出现（0 次或 1 次）用 `?` 表示。 `x ::= y?`表示可以是一个 y，也可以是空字符串
- 出现一次或多次用`+`表示，`x ::= y+`表示 x 是一个或多个 y，等价于`x ::= y y*`
- 指定确切出现次数或出现次数范围可以用`{n}`、`{n,m}`、`{n,}` 或 `{,m}` 表示：
  - `x ::= y{3}` 表示 x 是 3 个 y， 等价于`x ::= y y y`
  - `x ::= y{,4}` 表示 x 是至多 4 个 y， 等价于 x ::=   | y | y y | y y y | y y y y
- 字符类 `[ ... ]` 表示匹配方括号中任意一个字符的单字符字符串集合
  - `x ::= [aeiou]`     等价于：`x ::= 'a' | 'e' | 'i' | 'o' | 'u'`
- 可以用 `-` 来紧凑地表示字符范围：
  - `x ::= [a-ckx-z]`   等价于：`x ::= 'a' | 'b' | 'c' | 'k' | 'x' | 'y' | 'z'`
- 反向字符类 `[^ ... ]` 表示匹配**不在**方括号内列出的任意单个字符
  - `x ::= [^a-c]`，即所有不在 `a-c` 范围内的字符，例如 `d`、`0`、`!` 等
...
![URL 文法对应的语法树结构](https://tc-1258979383.cos.ap-guangzhou.myqcloud.com/image-20251020021420751.png)
...
### Markdown 与 HTML 的递归差异

为了简化起见，下面的示例文法只描述**斜体（italic）**。
...
::: insight
文法的价值不只是“检查字符串是否合法”，而是把序列格式写成可执行的接口规格：解析器负责验证并保留结构，后续代码再把语法树转换为领域数据。正则表达式适合局部、扁平的模式；一旦结构允许递归嵌套，就应使用真正的解析器，避免用越来越脆弱的 regex 模拟栈。
:::

## 尚待补充的解析器生成器

当前来源笔记在这里中断，尚未展开如何把文法交给解析器生成器、如何构造抽象语法树，以及如何将语法树转换为类型安全的数据结构。补充时应继续使用本讲的 URL、Markdown 或 HTML 文法作为贯穿示例，并与[领域特定语言](./lec19.md)建立联系。
