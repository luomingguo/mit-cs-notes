...
type: lecture
lecture: 2
tags: [html, semantic-web, dom, web-accessibility, responsive-design]
status: complete
---
# Lec 2 Web 网页结构：HTML
...
- 语义化 HTML 同时为浏览器、辅助技术、搜索工具和开发者描述内容角色，不能只用视觉外观代替结构。
- DOM 把文档表示为可遍历和可操作的对象树，CSS 选择器与 JavaScript 都以这棵树为共同接口。
- 原生表单控件与正确的标题、分区和链接结构通常自带更可靠的键盘行为与可访问性。
- 响应式界面既可由 CSS 适配布局，也可由 Vue 等模板系统让数据变化自动反映到页面。
...
如果用它来做布局，会改变 HTML 向其他解析工具传达的语义信息，对屏幕阅读器、搜索引擎机器人等尤其不友好。CSS 布局用 grid
...
- HTML 元素（*HTMLELEMENT*）：每一个 HTML 元素，例如 `<html>`、`<body>` 或 `<em>` 都属于这一类型。通常它们只是继承自 `HTMLElement`，并且是更具体类型的实例，例如 `HTMLHtmlElement`、`HTMLBodyElement` 等。
...
::: insight
语义 HTML 可以看成界面的公共 API：视觉样式会不断变化，但结构角色仍要被浏览器、脚本和辅助技术稳定理解。优先使用原生元素，往往比事后补齐交互与可访问性成本更低。
:::
