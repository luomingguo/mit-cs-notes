...
type: lecture
lecture: 23
tags: [web-standards, standardization, interoperability, specification]
status: complete
---
# Lec 23 Web 标准
...
- Web 生态由浏览器、开发者、标准组织和用户共同演化，兼容性依赖公开且可实现的规范。
- 标准化从问题、用例和提案出发，通过实现与测试验证规范能否互操作，而不是先固化单一产品方案。
- 个人可以通过阅读提案、提供用例、测试实现和参与开放讨论影响 Web 平台。
...
::: insight
标准的价值不是规定唯一实现，而是让不同实现共享可观察行为。测试因此不是收尾阶段，而是检验规范是否足够明确、完整且可互操作的证据。
:::
