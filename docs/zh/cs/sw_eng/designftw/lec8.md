...
type: lecture
lecture: 8
tags: [css, cascade, inheritance, css-specificity, css-custom-properties]
status: complete
---
# Lec 8 CSS 基本概念 II
...
- 元素的最终样式由继承、规则来源、重要性、选择器特异性和源码顺序共同决定。
- 简写属性会同时设置多个展开属性，使用前必须理解哪些值会被隐式重置。
- DRY 原则与 CSS 自定义属性把重复设计值集中为可复用变量，但变量命名仍应表达语义而非偶然数值。
...
::: insight
级联不是 CSS 的偶然复杂性，而是允许浏览器默认值、设计系统、组件和局部覆盖共同工作的冲突解决机制。可维护性的关键是控制覆盖层次，而不是试图消灭级联。
:::
