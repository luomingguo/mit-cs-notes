...
JavaScript 是一个单线程，但我们经常需要调用一些外部的、耗时较长的任务，或者是那些会根据用户输入而不可预测地触发的任务，比如定时器、DOM 事件、网络请求（例如加载图片、调用大语言模型）、以及数据库和文件访问等。
...
是通过函数来把执行控制权交给异步任务，我们其实两次见到这种回调模式了。那么当我们需要把多个一步任务 chuanlianshi
