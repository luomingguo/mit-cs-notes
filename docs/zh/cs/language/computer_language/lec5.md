...
::: definition 定义（加法的类型规则）
数值类型构成层级：$\text{int} \prec \text{float} \prec \text{double}$。所有强制转换**沿层级向上**（int→float；int、float→double）。**结果类型 = 层级中较高的那个操作数类型**：
$$\text{int}+\text{float}=\text{float},\quad \text{int}+\text{double}=\text{double},\quad \text{float}+\text{double}=\text{double}$$
:::

::: example 例题（有趣的历史怪癖）
C 语言会把 `float` 类型的**过程实参**转换成 `double`。原因与早期 C 不写函数原型、统一用 double 传浮点以简化调用约定有关。
:::
...
::: definition 定义（兼容性 / 可替换性）
赋值 `A = B` 必须检查类型兼容：**B 兼容于 A** 意味着——任何需要 A 的地方都能用 B 替代，B 满足 A 的所有要求，B 至少能做 A 能做的一切（Liskov 替换思想）。

- Int 兼容 Int；

- Int 与 Float 互相兼容（含强制转换）；

- 若 **D 继承自 C**，则 **类 D 兼容于 类 C**（反之不成立）。
:::
...
- 许多检查对应"确保构建正确 IR 所需的实体存在"（声明、查找）；

- 其余对应简单理智检查（重复名、类型兼容、参数匹配）；

- 每门语言都有一张必须检查的清单，可在编译期标记大量潜在错误。
:::
