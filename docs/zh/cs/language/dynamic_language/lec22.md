...
- **线性扫描 (Linear Scan)**——快、简单，适合 JIT；

- **图着色 (Graph Coloring)**——质量高、更通用，适合 AOT/优化编译。
...
::: definition 定义（干涉 Interference）
两个变量**干涉**，若它们在某程序点**同时活跃**（活跃区间重叠）——则它们不能分配到同一寄存器。
:::

::: definition 定义（干涉图 Interference Graph）
结点 = 变量（虚拟寄存器/临时值）；无向边 = 两变量干涉。**给 k 个寄存器分配 = 用 k 种颜色给干涉图着色，使相邻结点异色**。这正是图论的 **k-着色问题**。
:::
...
::: theorem 定理（度数 &lt; k 必可着色）
若干涉图中某结点的度数（邻居数）**小于可用寄存器数 k**，那么无论其邻居如何着色，总能给它留出一种颜色。

于是可**移除**这种低度结点（连同它的边），递归着色剩下的图，最后再把它放回并染色。
:::

### 3.1 算法步骤（Chaitin-Briggs）

::: definition 定义（图着色分配流程）
- **Build（建图）**：由活跃性分析建干涉图；

- **Simplify（化简）**：反复移除度数 &lt; k 的结点，压入栈；

- **Spill（溢出候选）**：若只剩度数 ≥ k 的结点，选一个标记为**潜在溢出**并移除（也压栈），继续 simplify；

- **Select（着色）**：从栈逐个弹出结点放回图，分配一个与邻居不冲突的颜色；若某潜在溢出结点放回时无可用颜色 → **实际溢出 (actual spill)**；

- **Spill code（溢出代码）**：为实际溢出的变量插入存/取内存指令，然后**重建图重跑**。
:::

### 3.2 溢出（Spilling）

::: definition 定义（Spill）
寄存器不够时，把某变量放到内存（栈帧槽），在每次使用前 load、定义后 store。**选谁溢出**是启发式：通常选**溢出代价低 / 度数高**者（如使用次数少、活跃区间长、不在循环内的变量）。溢出降低图的度数，使其变得可着色。
:::

### 3.3 寄存器合并（Coalescing）

::: definition 定义（Coalescing 合并）
对于 `a = b` 这样的**复制 (move)**，若 a 与 b 不干涉，可把它们**合并成一个结点**分配到同一寄存器，从而**消除这条 mov**（这正是 L21 里"为什么这么多 mov"的解药）。

风险：合并会提高结点度数、可能让图变得不可着色。George–Appel 的**迭代寄存器合并 (Iterated Register Coalescing, 1996)** 用保守判据（Briggs/George）只在不破坏可着色性时才合并。
:::

::: theorem 定理（图着色的复杂度）
一般 k-着色是 **NP 完全**的，故实用分配器用启发式（Simplify/Coalesce/Freeze/Spill 的迭代）。质量高但编译较慢——适合 AOT/优化编译。
:::
...
::: definition 定义（线性扫描）
把每个变量近似为一个**活跃区间 [start, end]**（L21 定义的 live interval）。按 **start 排序**，从前往后扫描；维护一个按 **end 排序的活跃区间集合 (active list)**：

- 遇到新区间，先**expire**（淘汰）所有 end 已早于当前 start 的旧区间，释放它们的寄存器；

- 若有空闲寄存器，分配给新区间，加入 active；

- 若无空闲寄存器 → 在新区间与 active 中 end 最晚者之间选一个**溢出**（通常溢出 end 更晚的那个）。
:::

::: theorem 定理（线性扫描的取舍）
- **优点**：近似线性时间、实现简单、内存开销小 → JIT 首选（早期 HotSpot client、V8 等）；

- **缺点**：把活跃范围粗化为单一连续区间，**精度低于图着色**，可能多溢出；有诸多改进（区间带"洞"的版本、second-chance、SSA 上的线性扫描）。
:::
...
::: theorem 定理（Linear Scan vs Graph Coloring）
<table>
...
::: theorem 定理（分配必须遵守的约束）
- 每个程序点每个变量唯一位置；同一变量可在不同点位于不同寄存器/内存；

- 不同时活跃的变量可共用一个寄存器（这正是分配能"省"的根源）；

- 必须尊重 ISA 约束：结果固定落某寄存器的指令、**caller/callee-saved** 约定（跨 call 要保存的寄存器需在调用前后 push/pop 或避免使用）；

- 溢出代码必须在每次 use 前 load、def 后 store，保证语义不变。
:::
