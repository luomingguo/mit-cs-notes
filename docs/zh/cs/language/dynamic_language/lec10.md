...
::: definition 定义（闭包带来的核心难题）
一个闭包可能引用某个**已经被弹出栈**的作用域中的变量——作用域的帧可能比"该作用域代码的单次执行"**活得更久 (long-lived)**。纯栈式实现（L9）无法支持。
:::

::: example 例题（捕获的变量在块结束后仍存活）
```text
var f = 0;
{ var x = 1;
  f = fun() { print(x); x = x + 1; };
...
每次调用 `g()` 都新建一个 x，故每个返回的闭包**各有独立的 x** → 1,1,1（对比复用同一闭包的 1,2,3）。C 的 static/全局做不到这种"每实例独立"。
:::
...
::: definition 定义（链式栈 / 堆上帧）
不再用连续栈，而是**把每个帧作为堆对象分配**：

- **帧 (frame)** = 堆中的一个记录，含若干"变量 → 地址"绑定，外加一个**父指针 `p`** 指向外层（enclosing）帧的地址。

- **帧指针 (frame pointer, FP) $a$** = 当前帧在堆中的地址。

- 帧由父指针链成一条链（"链式栈"，又称仙人掌栈 cactus stack）——一个帧可被多个子帧/闭包共享，故能在"逻辑栈帧弹出"后继续存活。

求值关系变为 $\langle e, a, h\rangle \Downarrow \langle a', h'\rangle$："给定帧指针与堆"求值。
:::

::: example
**例题（链式栈堆轨迹：`var x=1; var y=2; { var x=3; y=4; }`）**
...
要点：块帧 108 的父指针 `p:100` 把它链到外层；`y=4` 通过 lookup 沿链找到外层的 y 并就地更新。
:::
...
::: definition 定义（闭包值的表示）
闭包值是三元组 $(a_{\text{captured}},\ \text{params},\ \text{body})$：

- $a_{\text{captured}}$：**创建时捕获的帧指针**（定义处的环境）；

- $\text{params}$：形参；$\text{body}$：函数体代码。

如堆中 `116 : (208, y, {print(x); x = x + 1})` 表示一个捕获帧 208、形参 y、体为 `{print(x); x=x+1}` 的闭包。
:::
...
- **创建新帧**，其**父指针指向闭包捕获的帧 $a_c$**（不是调用者的帧！），并把形参绑定到实参；

- 以新帧为 FP **求值闭包体** $s$；返回其结果。

"父指针 = 捕获帧"正是词法作用域：闭包体里的自由变量沿着**定义时的链**解析，而非调用时的链。
:::
...
输出 **1, 2**（再调一次得 3）。两次调用的实参帧不同（212/216），但**父指针都指向同一捕获帧 208**，所以 x 被持续累加——这正是"每个闭包一份、跨调用持久"的捕获语义。
:::
