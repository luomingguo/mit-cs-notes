...
观察到，𝛿\_k(s, v) = 𝛿\_{k-1}(s, u) + w(u, v)，这个递归不存在循环。如果随着 k 增长，都是满足上述等式，
...
给定序列$t_0, t_1, ...t_{n-1}$ 单音符，用右手弹奏，右手手指分别是 1,2，...，f，给定过渡从音符 t 及其手指 f，到音符 t'及其手指 f'的难度度量$d(t, f, t', f')$
...
- Subproblem
  - $x(i, f)$ 最小化的演奏难度分，弹奏音符为$t_i, t_{i+1}, ..., t_{n-1}$，并且手指 f 在字符$t_i$
