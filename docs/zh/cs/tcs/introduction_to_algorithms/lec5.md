...
假设： 所有键的范围为$\{0, ..., u-1 \}$内唯一的非负整数，因此有 $n \le u$；将每个元素插入大小为 u 的直接访问数组，时间复杂为$\Theta(n)$，以直接访问数组中的顺序返回元素，时间复杂度为$\Theta(u)$， 总的时间复杂度为$\Theta(n+u)$。当 u  = $\Theta(n)$时，则时间复杂度为$\Theta(n)$
...
- python 中``divmod(k, n)``进行拆分可得到元组（a, b）
- 对于 `[17, 3, 24, 22, 12]`，则 n = 5 我们得到 `[(3,2), (0,3), (4,4), (4,2), (2,2)]` ，即按 n=5 得出 `[32, 03, 44, 42, 22]`
